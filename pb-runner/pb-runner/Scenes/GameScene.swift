...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    private var gameData: GameData? {
        ...
        private var unlockedSkins: [String] {
            get { gameData?.unlockedSkins ?? ["jump"] }
            set {
                gameData?.unlockedSkins = newValue
        ...
        private var currentScore: Int {
            get { gameData?.currentScore ?? 0 }
            set {
                gameData?.currentScore = newValue
                GameDataManager.shared.save()
            }
        }
    ...
    @AppStorage("selectedSkin") private var selectedSkin: String = "jump"
    @AppStorage("selectedBackgroundSkin") private var selectedBackgroundSkin = "LesArcs"
    @AppStorage("backgroundNeedZoom") private var backgroundNeedZoom = false
    
    private var audioManager = AudioManager.shared
    
    private var unlockAnimationView: SKView?
    ...
    private var backgroundLayers: [SKSpriteNode] = []// remplacer par pviewportBackground: ViewportBackground?
    
    private var viewportBackground: ViewportBackground?
    ...
    private var unlockAnimationViewController: UIHostingController<UnlockAnimationView>?
    
    
    
    private let IS_TEST_MODE = false;
    ...
    func setup() {
        if !audioManager.same(as: currentBackground.mainTheme) {
            audioManager.stopBackgroundMusic()
            audioManager.playBackgroundMusic(track: currentBackground.mainTheme)
        }
        if selectedBackgroundSkin == "snow" {
            startSnowfall()
        }
    ...
    private func setupJumpButtons() {
        ...
        upButton.size = CGSize(width: 16 * 16, height: 16 * 16)
        if upOnTheRight{
        ...
        downButton.size = CGSize(width: 16 * 16, height: 16 * 16)
...
//        }
    ...
    private var currentBackground: BackgroundSkinParalax {
        // Trouver le background correspondant dans les skins
        let selector = BackgroundSelector(selectedBackgroundSkin: .constant(""), backgroundNeedZoom: .constant(false))
        return selector.skins.first(where: { $0.imageName == selectedBackgroundSkin }) ??
    ...
    private func setupBackground() {
        ...
        backgroundLayers.forEach { $0.removeFromParent() }
        backgroundLayers.removeAll()
        
        for i in 0..<currentBackground.numberOfLayers {
            let layerName = "\(selectedBackgroundSkin)_layer\(i+1)"
            let backgroundTexture = SKTexture(imageNamed: layerName)
            backgroundTexture.filteringMode = .nearest
            
            let backgroundLayer = SKSpriteNode(texture: backgroundTexture)
            print( "DEBUG: current background = " + currentBackground.imageName)
            
            if currentBackground.needZoom {
                backgroundLayer.size = CGSize(width: 256 * zoomFactorForMob * 10, height: 128 * zoomFactorForMob * 10)
            } else {
                if currentBackground.imageName == "Lascaux" {
                    backgroundLayer.size = CGSize(width: self.frame.width / 2, height: self.frame.height)
            ...
            backgroundLayer.position = CGPoint(x: self.frame.midX, y: self.frame.midY)
            backgroundLayer.zPosition = -CGFloat(currentBackground.numberOfLayers - i)
...
//        
    private func updateBackgroundParallax() {
        ...
        for (index, layer) in backgroundLayers.enumerated() {
            let layerIndex = index / 2 // Car chaque couche a une copie
            guard layerIndex < currentBackground.layerSpeeds.count else { continue }
            
            let layerSpeed = baseSpeed * currentBackground.layerSpeeds[layerIndex]
...
//            backgroundTexture.filteringMode = .nearest
//            
//            // Obtenir les dimensions réelles de la texture
//            let textureSize = backgroundTexture.size()
//            
//            let backgroundLayer = SKSpriteNode(texture: backgroundTexture)
//            if backgroundNeedZoom {
//                let zoomedWidth = textureSize.width * zoomFactorForMob
//                let zoomedHeight = textureSize.height * zoomFactorForMob
//                backgroundLayer.size = CGSize(width: zoomedWidth, height: zoomedHeight)
//            } else {
//                // Garder le ratio hauteur/largeur original
//                let heightScale = self.frame.height / textureSize.height
//                let scaledWidth = textureSize.width * heightScale
//                backgroundLayer.size = CGSize(width: scaledWidth, height: self.frame.height)
//            }
//            
//            backgroundLayer.position = CGPoint(x: self.frame.midX, y: self.frame.midY)
//            backgroundLayer.zPosition = -CGFloat(currentBackground.numberOfLayers - i)
//            
//            // Créer une copie seulement si nécessaire pour couvrir l'écran
//            let backgroundLayerCopy = backgroundLayer.copy() as! SKSpriteNode
//            backgroundLayerCopy.position = CGPoint(x: backgroundLayer.position.x + backgroundLayer.size.width, y: backgroundLayer.position.y)
//            
//            addChild(backgroundLayer)
...
//            layer.position.x -= layerSpeed
//            
//            // Réinitialiser la position quand la couche sort complètement de l'écran
//            if layer.position.x <= -layer.size.width / 2 {
//                layer.position.x += layer.size.width * 2
//            }
//        }
...
//    
    ...
    private func updateScore() {
        ...
        if score > highScore {
            highScore = score
            checkNewUnlockedSkins()
        }
    }
    
    private func updateArrowOpacity() -> CGFloat {
        if score <= 5 {
            return 1.0
        } else {
            return 0.3
    ...
    private func showUnlockAnimation(for skin: PlayerSkin) {
            let unlockView = UnlockAnimationView(skin: skin) {
                self.unlockAnimationViewController?.view.removeFromSuperview()
                self.unlockAnimationViewController?.removeFromParent()
                self.unlockAnimationViewController = nil
            }
            
            let viewController = UIHostingController(rootView: unlockView)
            viewController.view.frame = self.view!.bounds
            viewController.view.backgroundColor = .clear
            
            // Pour que les touches passent à travers la vue
            viewController.view.isUserInteractionEnabled = false
            
            self.unlockAnimationViewController = viewController
            
            self.view!.addSubview(viewController.view)
            self.view?.window?.rootViewController?.addChild(viewController)
            viewController.didMove(toParent: self.view?.window?.rootViewController)
        }
        
        private func checkNewUnlockedSkins() {
            for skin in PlayerSkinSelector.skins {
                if highScore >= skin.requiredScore && !unlockedSkins.contains(skin.imageName) {
                    var updatedSkins = unlockedSkins
                    updatedSkins.append(skin.imageName)
                    self.unlockedSkins = updatedSkins
                    // On ne met plus le jeu en pause
                    showUnlockAnimation(for: skin)
                    break
                }
            }
        }
